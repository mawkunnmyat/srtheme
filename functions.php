...
if ( ! function_exists( 'srtheme_setup' ) ) :
	...
	function srtheme_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'srtheme_custom_background_args',
				array(
...
function srtheme_scripts() {
	wp_enqueue_style( 'srtheme-style', get_stylesheet_uri(), array(), _S_VERSION );

	/*** Added by Poe Ei Phyu ****/
	wp_enqueue_style( 'srtheme-main', get_template_directory_uri() . '/css/main.css');
	wp_enqueue_style( 'bootstrap-icons', 'https://cdn.jsdelivr.net/npm/bootstrap-icons@1.4.0/font/bootstrap-icons.css');
	
	wp_style_add_data( 'srtheme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'srtheme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
